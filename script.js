...
function renderizarSugestao(saldo, nServsAtivos) {
    ...
    // CCs disponíveis excluindo CC-1 — ordenados do mais barato ao mais caro
    const ccMap = {};
    ...
    // Gera TODAS as combinações com repetição de tamanho 1 até vagasRestantes
    // que caibam no saldo — sem limite artificial de 9 sugestões
    const sugestoes = [];
    const vistos = new Set();

    function gerarCombinacoes(qtdSlots) {
        // Combinações com repetição: escolhe qtdSlots CCs (ordem não importa, repetição ok)
        function combinar(inicio, atual, custoAtual) {
            if (atual.length === qtdSlots) {
                if (custoAtual > saldo + TOLERANCIA) return;
            ...
            for (let i = inicio; i < ccs.length; i++) {
                ...
                if (novoCusto > saldo + TOLERANCIA) break; // ccs está ordenado, não adianta continuar
    ...
    // Ordena: primeiro por quantidade de cargos (desc), depois por custo (desc)
    sugestoes.sort((a, b) =>
        b.ccs.length !== a.ccs.length
            ? b.ccs.length - a.ccs.length
            : b.custo - a.custo
    );
    ...
    const cards = sugestoes.map(s => {
        ...
        // Agrupa CCs repetidos para exibição: CC-9 + CC-9 → 2× CC-9
        const contagem = {};
