...
function renderizarEstruturaCCs(estrutura, cargosAtivosCC) {
    ...
    ordem.forEach(cc => {
        ...
        // Todos os cards do mesmo CC ficam lado a lado em uma única linha
        const cardsHTML = lista.map(estado => `
            <div class="cc-card ${estado}">
                <span class="cc-card-label">${cc}</span>
                <span class="cc-card-status">${estado === 'ocupado' ? '● ocupado' : '○ vago'}</span>
            </div>`).join('');

        html += `<div class="grupo-cc"><div class="grupo-cc-linha">${cardsHTML}</div></div>`;
    });
