...
function renderizarEstruturaCCs(estrutura, cargosAtivosCC) {
    ...
    const CARD_W   = 85;  // px — largura do card
    const GAP_SAME = 6;   // px — espaço entre cards do mesmo CC
    const GAP_DIFF = 20;  // px — espaço entre grupos de CC diferentes
    const ROW_GAP  = 6;   // px — espaço entre linhas

    const maxSlots = Math.max(...ordem.map(cc => grupos[cc].length));

    // Constrói o template de colunas uma única vez — idêntico para todas as linhas
    // Estrutura: card [gap_same card]* gap_diff card [gap_same card]* gap_diff ...
    const colParts = [];
    ordem.forEach((cc, gi) => {
        const n = grupos[cc].length;
        for (let j = 0; j < n; j++) {
            if (j > 0) colParts.push(`${GAP_SAME}px`); // gap entre cards do mesmo CC
            colParts.push(`${CARD_W}px`);
        }
        if (gi < ordem.length - 1) colParts.push(`${GAP_DIFF}px`); // gap entre grupos
    });
    const templateCols = colParts.join(' ');

    // Para cada posição (linha), popula as células na mesma ordem das colunas
    let linhasHTML = '';
    for (let pos = 0; pos < maxSlots; pos++) {
        let cellsHTML = '';
        ordem.forEach((cc, gi) => {
            const n = grupos[cc].length;
            for (let j = 0; j < n; j++) {
                if (j > 0) cellsHTML += `<div></div>`; // gap_same spacer
                const estado = grupos[cc][pos];
                if (estado !== undefined) {
                    cellsHTML += `<div class="cc-card ${estado}">
                        <span class="cc-card-label">${cc}</span>
                        <span class="cc-card-status">${estado === 'ocupado' ? '● ocupado' : '○ vago'}</span>
                    </div>`;
                } else {
                    cellsHTML += `<div class="cc-card-vazio"></div>`;
                }
            }
            if (gi < ordem.length - 1) cellsHTML += `<div></div>`; // gap_diff spacer
        });
        linhasHTML += `<div class="linha-posicao" style="grid-template-columns:${templateCols}">${cellsHTML}</div>`;
    }

    return `<div class="grade-linhas-posicao" style="gap:${ROW_GAP}px">${linhasHTML}</div>`;
}
