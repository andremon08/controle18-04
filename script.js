...
function renderizarCardsSugestao(filtro) {
    const gridEl = document.getElementById('sugestaoGrid');
    const q = filtro.toUpperCase().replace(/\s/g, '');

    const filtradas = q
        ? _todasSugestoes.filter(s => s.ccs.some(cc => cc.replace('-', '').includes(q.replace('-', ''))))
        : _todasSugestoes;

    if (filtradas.length === 0) {
        gridEl.innerHTML = `<p class="sugestao-sem-resultado">Nenhuma combinação encontrada para "${filtro}".</p>`;
        return;
    }

    gridEl.innerHTML = filtradas.map(s => {
        const n = s.ccs.length;
        const saldoPos = saldo_atual - s.custo; // usa variável global do contexto
        const contagem = {};
