...
function renderizarSugestao(saldo, nServsAtivos) {
    ...
    // Vagas disponíveis pelo total legal (9), não pela estrutura atual
    const vagasRestantes = MAX_SERVIDORES - nServsAtivos;

    // Só mostra se há vagas livres E saldo positivo
    ...
    // Badge com concordância correta
    resumo.textContent = vagasRestantes === 1
        ? `1 vaga disponível — saldo ${moeda(saldo)}`
        : `${vagasRestantes} vagas disponíveis — saldo ${moeda(saldo)}`;

    // CCs disponíveis — exclui CC-1 (obrigatório, sempre ocupado)
    const ccMap = {};
    Object.entries(tabelaCargos).forEach(([nome, sal]) => {
        const cc = extrairCC(nome);
        if (!cc.startsWith('CC-')) return;
        if (cc === 'CC-1') return; // CC-1 sempre ocupado, nunca sugerido
        if (!ccMap[cc]) ccMap[cc] = sal;
    });
    const ccsSorted = Object.entries(ccMap)
        .map(([cc, sal]) => ({ cc, sal }))
        .sort((a, b) => a.sal - b.sal);

    if (ccsSorted.length === 0) {
        conteudo.innerHTML = `<p class="sugestao-intro">Tabela de cargos não carregada.</p>`;
        return;
    }

    // Gera combinações de 1 até vagasRestantes cargos dentro do saldo
    const sugestoes = [];

    // 1 cargo (qualquer quantidade)
    ...
    // 2 CCs diferentes (cada um com 1 unidade)
    if (vagasRestantes >= 2) {
        for (let i = 0; i < ccsSorted.length; i++) {
            for (let j = i + 1; j < ccsSorted.length; j++) {
    ...
    if (vagasRestantes >= 3) {
        for (let i = 0; i < ccsSorted.length; i++) {
            for (let j = i + 1; j < ccsSorted.length; j++) {
                for (let k = j + 1; k < ccsSorted.length; k++) {
                    const custo = ccsSorted[i].sal + ccsSorted[j].sal + ccsSorted[k].sal;
                    if (custo <= saldo + TOLERANCIA) {
                        sugestoes.push({ ccs: [ccsSorted[i].cc, ccsSorted[j].cc, ccsSorted[k].cc], custo });
                    }
                }
            }
        }
    ...
    // Ordena por custo decrescente, remove duplicatas, limita a 9
    ...
    const intro = `<p class="sugestao-intro">
        Com <strong>${vagasStr}</strong> e saldo de <strong>${moeda(saldo)}</strong>, os cargos abaixo podem ser incluídos na composição sem ultrapassar o teto legal de ${moeda(TETO_VEREADOR)}:
    </p>`;
    ...
    const cards = unicas.map(s => {
        ...
        const cargoStr = n === 1 ? '1 cargo adicional' : `${n} cargos adicionais`;
        return `
