...
function renderizarEstruturaCCs(estrutura, cargosAtivosCC) {
    ...
    // Monta grid-template-columns:
    // Cada grupo de CC tem N colunas de 85px com gap 6px entre elas,
    // e entre grupos diferentes um gap maior de 18px.
    // Usamos um gap global de 6px e simulamos o gap maior com uma coluna vazia de 12px.
    // Colunas: [cc1_slot1] [cc1_slot2?] [spacer] [cc2_slot1] ...
    const colDefs = [];
    ordem.forEach((cc, i) => {
        const n = grupos[cc].length;
        for (let j = 0; j < n; j++) colDefs.push('85px');
        if (i < ordem.length - 1) colDefs.push('12px'); // spacer entre grupos
    });
    const templateCols = colDefs.join(' ');

    // Para cada posição (linha), monta as células na ordem das colunas
    let linhasHTML = '';
    for (let pos = 0; pos < maxSlots; pos++) {
        ...
        ordem.forEach((cc, i) => {
            ...
            if (estado !== undefined) {
                ...
                // Slot vazio — mantém alinhamento de coluna
                cellsHTML += `<div class="cc-card-vazio"></div>`;
            }
            // Spacer entre grupos
            if (i < ordem.length - 1) {
                cellsHTML += `<div></div>`; // ocupa a coluna spacer de 12px
            }
        });
        linhasHTML += `<div class="linha-posicao" style="grid-template-columns:${templateCols};gap:6px">${cellsHTML}</div>`;
    }

    return `<div class="grade-linhas-posicao">${linhasHTML}</div>`;
}
