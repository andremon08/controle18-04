...
function renderizarEstruturaCCs(estrutura, cargosAtivosCC) {
    ...
    // Determina o número máximo de slots em qualquer CC (= número de linhas de posição)
    const maxSlots = Math.max(...ordem.map(cc => grupos[cc].length));

    // Monta uma linha por posição (1ª vaga de cada CC, 2ª vaga de cada CC, etc.)
    let linhasHTML = '';
    for (let pos = 0; pos < maxSlots; pos++) {
        const cardsNaLinha = ordem
            .filter(cc => grupos[cc][pos] !== undefined)
            .map(cc => {
                const estado = grupos[cc][pos];
                return `<div class="cc-card ${estado}">
                    <span class="cc-card-label">${cc}</span>
                    <span class="cc-card-status">${estado === 'ocupado' ? '● ocupado' : '○ vago'}</span>
                </div>`;
            }).join('');
        linhasHTML += `<div class="linha-posicao">${cardsNaLinha}</div>`;
    }
